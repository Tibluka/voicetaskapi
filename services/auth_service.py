...
class AuthService:
    ...
    def create_user(self, email, password, name, phone):
        ...
        activation_link = (
            f"https://voicetaskapi.onrender.com/auth/activate?code={code}&email={email}"
        )
    ...
    def change_user_password(
        ...
        self.user_collection.update_one(
            {"_id": ObjectId(user_id)}, {"$set": {"password": new_password_hash}}
        )
    ...
    def initiate_reset(self, email: str, force=False):
        ...
        if not user:
            return {
                "message": "Se esse e-mail existir, você receberá instruções para reset."
            }
        ...
        if existing_token and not force:
            return {
                "message": "Já existe um código ativo enviado recentemente. Verifique seu e-mail."
            }
    ...
    def reset_password(self, email: str, code: str, new_password: str):
        ...
        self.user_collection.update_one(
            {"_id": pr["userId"]}, {"$set": {"password": new_password_hash}}
        )
        self.password_resets.update_one({"_id": pr["_id"]}, {"$set": {"used": True}})
